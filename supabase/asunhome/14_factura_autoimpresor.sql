...
-- RLS: filtra por empresa_id igual a la del JWT (mismo patrón que las demás).
...
CREATE POLICY factura_autoimpresor_by_empresa ON asunhome.factura_autoimpresor
  FOR ALL
  TO authenticated
  USING (empresa_id = (current_setting('request.jwt.claims', true)::json ->> 'empresa_id')::uuid)
  WITH CHECK (empresa_id = (current_setting('request.jwt.claims', true)::json ->> 'empresa_id')::uuid);
