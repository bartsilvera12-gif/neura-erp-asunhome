...
export default function EditarPresupuestoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRetencionPct(Number(p.retencion_iva_pct) > 0 ? String(Number(p.retencion_iva_pct)) : "");
        setItems(its.map((it) => ({
  ...
  const totales = useMemo(() => {
    ...
    return { subtotal: round2(subtotal), iva: round2(iva), desc: round2(desc), total: round2(total), retMonto, neto };
  }, [items, retencionPct]);
        ...
        {items.length > 0 && (
          <div className="mt-4 ml-auto w-full sm:w-72 text-sm space-y-1">
            ...
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="retencion-iva-editar" className="flex items-center gap-1.5 text-gray-500">
                Retención IVA
                <span className="relative inline-flex items-center">
                  <input
                    id="retencion-iva-editar"
                    type="number"
                    min={0}
                    max={100}
                    step="1"
